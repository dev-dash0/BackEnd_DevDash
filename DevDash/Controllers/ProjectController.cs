    ...
    public class ProjectController : ControllerBase
    {
        ...
        public async Task<ActionResult<APIResponse>> GetProjects([FromQuery] int tenantid, [FromQuery] string? search, int pageSize = 0, int pageNumber = 1)
        {
            try
            {
                ...
                var projects = await _dbProject.GetAllAsync(
                    filter: p =>
                        p.TenantId == tenantid &&
                        (p.CreatorId == parsedUserId ||
                         p.UserProjects.Any(up => up.UserId == parsedUserId)) &&
                        (string.IsNullOrEmpty(search) ||
                         p.Name.ToLower().Contains(search.ToLower()) ||
                         p.Priority.ToLower().Contains(search.ToLower())),
                    includeProperties: "Tenant,UserProjects",
                    pageSize: pageSize,
        ...
        public async Task<ActionResult<APIResponse>> GetProject([FromRoute] int projectId)
        {
            try
            {
                ...
                var userId = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier)?.Value;
                var userProject = await _dbUserProject.GetAsync(ut => ut.UserId == int.Parse(userId) && ut.ProjectId == projectId);
                ...
                Project project = await _dbProject.GetAsync(u => u.Id == projectId, includeProperties:"Users,Creator");
                if (project == null)
                {
        ...
        public async Task<ActionResult<APIResponse>> UpdateProject([FromRoute] int projectId, [FromBody] ProjectUpdateDTO updateDTO)
        {
            try
            {
                if (updateDTO == null || projectId != updateDTO.Id)
                {
                    return BadRequest();
                }
