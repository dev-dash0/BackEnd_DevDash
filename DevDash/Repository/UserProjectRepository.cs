...
namespace DevDash.Repository
{
    public class UserProjectRepository : JoinRepositroy<UserProject>, IUserProjectRepository
    {
        ...
        public async Task JoinAsync(UserProject entity, string userId)
        {
            ...
            string message = userId == project.CreatorId.ToString()
        ...
        public async Task LeaveAsync(UserProject entity, string userId)
        {
            Project? project = await _db.Projects.FirstOrDefaultAsync(p => p.Id == entity.ProjectId);
            if (project == null) return;

            var assignedIssues = await _db.IssueAssignedUsers
                .Where(iau => iau.UserId.ToString() == userId && _db.Issues.Any(issue => issue.Id == iau.IssueId && issue.ProjectId == entity.ProjectId))
                .ToListAsync();
